...
{
#ifdef CONFIG_L4
	l4_vm_vmx_clear(l4x_current_vmcs(), (void*)vmcs);
#else
...
{
#ifdef CONFIG_L4
	l4_vm_vmx_ptr_load(l4x_current_vmcs(), (void*)vmcs);
#else
	u64 phys_addr = __pa(vmcs);
	bool error;
...
{
#ifdef CONFIG_L4
	return l4_vm_vmx_read(l4x_current_vmcs(), field);
#else
	unsigned long value;
...
{
#ifdef CONFIG_L4
	l4_vm_vmx_write(l4x_current_vmcs(), field, value);
#else
	bool error;
...
#ifdef CONFIG_L4
		l4x_set_guest_msr(vmx->guest_msrs[i].index,
	...
	default:
		...
		if (msr) {
			...
			if (msr - vmx->guest_msrs < vmx->save_nmsrs) {
				preempt_disable();
#ifdef CONFIG_L4
				l4x_set_guest_msr(msr->index, msr->data,
				                  msr->mask);
#else /* L4 */
...
static int handle_xsetbv(struct kvm_vcpu *vcpu)
{
...
#ifdef CONFIG_L4
	{
		...
		return kvm_skip_emulated_instruction(vcpu);
#ifdef CONFIG_L4
		l4_vm_vmx_write_nat(l4x_current_vmcs(), L4_VM_VMX_VMCS_XCR0,
		                    vcpu->arch.xcr0);
	}
#endif /* L4 */
	return 1;
